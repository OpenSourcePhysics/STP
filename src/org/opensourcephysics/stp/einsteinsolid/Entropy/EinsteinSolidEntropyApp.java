...
package org.opensourcephysics.stp.einsteinsolid.Entropy;
 ...
 */
public class EinsteinSolidEntropyApp extends AbstractCalculation implements InteractiveMouseHandler {
  ...
  public void addPoints() {
    for(int i = 0; i<=E; i++) {
      ...
      if((Ea+Na-1)>MAXIMUM_NUMERATOR) {
        ...
        log_omegaA = Math.log(EinsteinSolidEntropyApp.factorial(Ea+Na-1, Ea, Na-1));
      }
      if((Eb+Nb-1)>MAXIMUM_NUMERATOR) {
        log_omegaB = MyMath.stirling(Eb+Nb-1)-MyMath.stirling(Eb)-MyMath.stirling(Nb-1);
      } else {
        log_omegaB = Math.log(EinsteinSolidEntropyApp.factorial(Eb+Nb-1, Eb, Nb-1));
      }
  ...
  public static void main(String[] args) {
    CalculationControl.createApp(new EinsteinSolidEntropyApp(), args);
  }
