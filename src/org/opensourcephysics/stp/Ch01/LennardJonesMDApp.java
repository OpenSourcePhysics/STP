...
/**
 * LJfluidApp simulates a two-dimensional system of interacting particles via
...
public class LennardJonesMDApp extends AbstractSimulation {
	...
	public void initialize() {
		...
		md.nstep = (int) (control.getInt("maximum time for diffusion") / md.dt);
		md.ncstep = (int) (control.getInt("maximum time for velocity autocorrelation") / md.dt);
		md.initialize();
		gr.reset();
	...
	public void reset() {
		...
		control.setAdjustableValue("time step", 0.01);
		control.setValue("maximum time for diffusion", 10);
		control.setValue("maximum time for velocity autocorrelation", 10);
	 ...
	 * 
	 * public void switchGUI() { stopSimulation(); Runnable runner = new Runnable()
	 * { public synchronized void run() { OSPRuntime.disableAllDrawing = true;
	 * OSPFrame mainFrame = getMainFrame(); XMLControlElement xml = new
	 * XMLControlElement(getOSPApp()); WindowListener[] listeners =
	 * mainFrame.getWindowListeners(); int closeOperation =
	 * mainFrame.getDefaultCloseOperation();
	 * mainFrame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
	 * mainFrame.setKeepHidden(true); mainFrame.dispose(); LennardJonesWRApp app =
	 * new LennardJonesWRApp(); LJfluidControl c = new LJfluidControl(app,
	 * app.displayFrame, null);
	 * c.getMainFrame().setDefaultCloseOperation(closeOperation); for(int i = 0, n =
	 * listeners.length; i<n; i++) {
	 * if(listeners[i].getClass().getName().equals("org.opensourcephysics.tools.Launcher$FrameCloser"))
	 * { c.getMainFrame().addWindowListener(listeners[i]); } } c.loadXML(xml, true);
	 * app.customize(); c.resetSimulation(); System.gc();
	 * OSPRuntime.disableAllDrawing = false; GUIUtils.showDrawingAndTableFrames(); }
	 * 
	 * }; Thread t = new Thread(runner); t.start(); }
	 * 
	 * void customize() { OSPFrame f = getMainFrame();
	 * if((f==null)||!f.isDisplayable()) { return; } JMenu menu =
	 * f.getMenu("Display"); JMenuItem item = new JMenuItem("Switch GUI");
	 * item.addActionListener(new ActionListener() { public void
	 * actionPerformed(ActionEvent e) { switchGUI(); }
	 * 
	 * }); menu.add(item); addChildFrame(displayFrame);
	 * addChildFrame(xVelocityHistogram); addChildFrame(grFrame);
	 * addChildFrame(pressureData); addChildFrame(temperatureData); }
	 */
	public static void main(String[] args) {
		LennardJonesMDApp app = new LennardJonesMDApp();
 ...
 * 
 * Copyright (c) 2020 The Open Source Physics project
 * 
 * http://www.opensourcephysics.org
 */
