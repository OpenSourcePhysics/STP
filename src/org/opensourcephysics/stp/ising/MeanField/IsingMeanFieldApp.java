...
package org.opensourcephysics.stp.ising.MeanField;
...
public class IsingMeanFieldApp extends AbstractCalculation implements Function {
	PlotFrame plotFrame = new PlotFrame("m", "tanh(m)", "");
	PlotFrame zmFrame = new PlotFrame("M", "F(M)", "F(M) versus M");
	...
	public void calculateFullyConnected(){
	  //public void calculate() {
		...
		double JN = 4.0/N;
		double beta = .2;
		for (int M = -N; M < N+1; M += 2){
			double lnp = beta*(-0.5*JN*(N - M*M) +H*M);
			double omega = fact(N)/(fact((N/2) + (M/2))*fact((N/2) - (M/2)));
			double lnZm = lnp + Math.log(omega);
			System.out.println(M + " " + omega + " " + lnp);
			zmFrame.append(0,M,-lnZm);
		}
	...
	public static void main(String[] args) {
		CalculationControl.createApp(new IsingMeanFieldApp(), args);
	}
