 ...
 *
 */
public class HardDisksMD implements Drawable {
  double[] velocityX;
  double[] velocityY;
  double[] positionX;
  double[] positionY;
  ...
  double time, tenCollisionTime, tenCollisionVirial,ctime;
  double velocityMax;
  double area;
  ...
  Histogram colTimeHistogram = new Histogram();
  PlotFrame x2y2Frame = new PlotFrame("time", "x2,y2", "X2,Y2 averaged over all particles");
  /**
   * Constructor HD
   */
  public HardDisksMD() {
  ...
  public void saveHistogram(PlotFrame histFrame){
	  histFrame.clearData();
	  try {
		  PrintWriter outFile = new PrintWriter("histogramData.txt");
		  double CollisionTimes[],Histogram[];
		  CollisionTimes = colTimeHistogram.getXPoints();
		  Histogram = colTimeHistogram.getYPoints();
		  for(int i = 0; i < CollisionTimes.length;i++){
			if(Histogram[i] > 0){
		      outFile.printf("%7.4f \t %7.4f %n", CollisionTimes[i],Histogram[i] );
		      histFrame.append(0,CollisionTimes[i],Histogram[i]);
			}
		  }
		outFile.close();
  ...
  public void initialize() {
    if(initialConfiguration.equals("solid")) {
      setCrystalPositions();
    } else {
      setRandomPositions();
    }
    ...
    colTimeHistogram.setBinWidth(0.001); // assuming
    //colTimeHistogram.setBinOffset(velocityMax/N);
    for(int k = 0; k<N; k++) {
      positionX[k] = pbc(positionX[k], Lx);
      positionY[k] = pbc(positionY[k], Ly);
    }
    tmax = 2*Lx/velocityMax;
    delt = tmax/20;
    dx = new double[N];
    dy = new double[N];
    dx2 = new double[20];
  ...
  public void plotX2Y2(){
	  x2y2Frame.clearData();
	  for(int i = 0; i<20;i++){
		  if(ndxdy[i] > 0){
			  x2y2Frame.append(0,i*delt,dx2[i]/(ndxdy[i]*N));
			  x2y2Frame.append(1,i*delt,dy2[i]/(ndxdy[i]*N));
		  }
	  }
  }
  ...
  public Histogram getColTimeHistogram() {
	    return colTimeHistogram;
	  }
  ...
  public void move(double tij) {
	if(tD + tij <tmax){
		tD = tD +tij;
		int index = (int)(tD/delt);
		ndxdy[index]++;
		for(int k = 0; k<N; k++){
		      dx[k] += velocityX[k]*tij;
		      dy[k] += velocityY[k]*tij;
		      dx2[index] += dx[k]*dx[k];
		      dy2[index] += dy[k]*dy[k];
		}
	}
  ...
  public boolean checkOverlap() {
    double tol = .00001;
    boolean overlapParticles = false;
    for(int i = 0; i<N-1; i++) {
      for(int j = i+1; j<N; j++) {
        double dx = seperation(positionX[i]-positionX[j], Lx);
        double dy = seperation(positionY[i]-positionY[j], Ly);
        ...
        if(r2<1) {
          ...
          double dist = 1-r;
          if(dist>tol) {
            System.err.println("particles "+i+"  "+j+" overlap " + dist + " " + positionX[i] + " " + positionY[i] + " ," +  positionX[j] + " " + positionY[j]);
          
          overlapParticles = true;
          while(overlapParticles){
        	positionX[i] = pbc (positionX[i] + Math.random()-0.5,Lx);
        	positionY[i] = pbc (positionY[i] + Math.random()-0.5,Ly);
        	overlapParticles = checkOverlap();
          }
          }
        }
      }
    }
    return overlapParticles;
  }
