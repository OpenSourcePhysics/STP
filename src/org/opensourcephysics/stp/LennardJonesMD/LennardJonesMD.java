...
package org.opensourcephysics.stp.LennardJonesMD;
import java.awt.*;
 ...
 */
public class LennardJonesMD implements Drawable, ODE {
  public double state[];
  public double ax[], ay[];
  ...
  public double dx[],dy[],x2sum[],y2sum[],x[],y[];
  public double cxsum[],cysum[],vx0[],vy0[];
  public int istep,nstep,r2sum,csum,icstep,ncstep;
  public PlotFrame r2Plot = new PlotFrame("time", "<x2> and <y2>", "Mean square displacement");
  public PlotFrame vAutoPlot = new PlotFrame("time", "Cx(t) and Cy(t)", "Velocity Autocorrelation Function");
  Trail trail = new Trail();

  public void initialize() {
	r2Plot.setXYColumnNames(0,"time","x2");
	r2Plot.setXYColumnNames(1,"time","y2");
	vAutoPlot.setXYColumnNames(0,"time","cx(t)");
	vAutoPlot.setXYColumnNames(1,"time","cy(t)");
    N = nx*ny;
    istep = 0;
    icstep = 0;
    t = 0;
    ...
    vx0 = new double[N];
    vy0 = new double[N];
    x2sum = new double[nstep];
    y2sum = new double[nstep];
    cxsum = new double[ncstep];
    cysum = new double[ncstep];
  ...
  public void step(HistogramFrame xVelocityHistogram) {
    ...
    for(int i = 0; i<N; i++) {
      ...
      dx[i] += state[4*i] - x[i];
      dy[i] += state[4*i+2] - y[i];
      x2sum[istep] += dx[i]*dx[i];
      y2sum[istep] += dy[i]*dy[i];
      cxsum[icstep] += vx0[i]*state[4*i+1];
      cysum[icstep] += vy0[i]*state[4*i+3];
    ...
    icstep++;
    if(istep==nstep)walk();
    if(icstep==ncstep)vAutoCorrelation();
   }
  ...
  public void walk(){
	  ...
	  for(int i = 0; i < nstep;i++){
		  double x2ave = x2sum[i]/(N*r2sum);
		  double y2ave = y2sum[i]/(N*r2sum);
		  r2Plot.append(0,i*dt,x2ave);
		  r2Plot.append(1,i*dt,y2ave);
	  }
	  for(int i = 0; i < N;i++){
		  dx[i] =0;
		  dy[i] = 0;
  ...
  public void vAutoCorrelation(){
	  vAutoPlot.clearData();
	  csum++;
	  for(int i = 0; i < ncstep;i++){
		  double cxave = cxsum[i]/(N*csum);
		  double cyave = cysum[i]/(N*csum);
		  vAutoPlot.append(0,i*dt,cxave);
		  vAutoPlot.append(1,i*dt,cyave);
	  }
	  for(int i = 0; i < N;i++){
		  vx0[i] =state[4*i+1];
		  vy0[i] =state[4*i+3];
  ...
  public void draw(DrawingPanel panel, Graphics g) {
    ...
    int xpix = panel.xToPix(state[2*(N+nx)])-pxRadius;
    int ypix = panel.yToPix(state[2*(N+nx)+2])-pyRadius;
    g.fillOval(xpix, ypix, 2*pxRadius, 2*pyRadius);
   // trail.addPoint(state[2*(N+nx)], state[2*(N+nx)+2]);
   // trail.draw(panel, g);
    g.setColor(Color.red);
    for(int i = 0; i<N; i++) {
    	if(i != (N+nx)/2){
           xpix = panel.xToPix(state[4*i])-pxRadius;
           ypix = panel.yToPix(state[4*i+2])-pyRadius;
           g.fillOval(xpix, ypix, 2*pxRadius, 2*pyRadius);
    	}
