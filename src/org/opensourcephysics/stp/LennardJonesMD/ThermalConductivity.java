 ...
 */
public class ThermalConductivity implements  ODE {
  public double state[];
...
//	      for(int j = i+1; j<N; j++) {
  ...
  public void step(boolean equil) {
	    ...
	    t += dt;
	    if(steps % stepAdd == 0 && !equil) swapHotandCold();
	    if(steps % 12 == 0) setNNList();
	    if((steps % 10 == 0) && equil)scaleVelocities();
  }
  
  public void swapHotandCold() {
	  double hot = 0;
	  double cold = 1E10;
	  int ihot = 0;
	  int icold = 0;
	  for(int j = 0; j < nSlab[0]; j++){
		  int i = slab[0][j];
		  double v2 = (state[6*i+1]*state[6*i+1]+state[6*i+3]*state[6*i+3]+state[6*i+5]*state[6*i+5]);
		  if(v2 > hot){
			  hot = v2;
			  ihot = i;
		  }
	  ...
	  double vx = state[6*ihot+1];
	  double vy = state[6*ihot+3];
	  double vz = state[6*ihot+5];
	  state[6*ihot + 1] = state[6*icold +1];
	  state[6*ihot + 3] = state[6*icold +3];
	  state[6*ihot + 5] = state[6*icold +5];
	  state[6*icold +1] = vx;
	  state[6*icold +3] = vy;
	  state[6*icold +5] = vz;  
