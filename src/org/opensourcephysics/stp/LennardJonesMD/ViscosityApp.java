 ...
 */ 
public class ViscosityApp extends AbstractSimulation {
  Viscosity md = new Viscosity();
  ...
  PlotFrame slabMomentum = new PlotFrame("z (distance from cold slab)", "<vx>", "<vx> in each slab");
  PlotFrame slabDensity = new PlotFrame("z (distance from cold slab)", "density", "Number density of each slab");
  double[] vxslab, rhoslab;
  int nvxslab[];
  double dvxdz;
  boolean equil = true;
  ...
  public void doStep() {
    ...
    for(int i = 0; i<md.N; i++) {
       temperature += md.state[6*i+1]*md.state[6*i+1]+md.state[6*i+3]*md.state[6*i+3]+md.state[6*i+5]*md.state[6*i+5];
       int index = (int)(md.state[6*i+4]/(md.a));
       vxslab[index] += md.state[6*i+1];
       rhoslab[index]++;
       nvxslab[index]++;
    }
    temperature /= 3*md.N;
    ...
    for(int islab = 1; islab < 3*md.n;islab++){
    	...
    	if(islab > 3*md.n/2)
    		z = md.n*3*md.a - z; 		
    	if(islab != 3*md.n/2){
    		double y = vxslab[islab]/nvxslab[islab]; 
      		slabMomentum.append(0,z,y);
 ...
 //   control.println("Heat capacity = "+decimalFormat.format(md.getHeatCapacity()));
    ...
    control.println("d<vx>/dz = "+decimalFormat.format(dvxdz) + " transfer = "+decimalFormat.format(md.transfer));
    control.println("viscosity = "+decimalFormat.format(md.transfer/(2*md.dt*md.steps*md.Lx*md.Ly*dvxdz)));
     }
  ...
  public void resetData() {
    md.resetAverages();
    vxslab = new double[3*md.n];
    rhoslab = new double[3*md.n];
    nvxslab = new int[3*md.n];
  ...
  */
  ...
  public static void main(String[] args) {
	ViscosityApp app = new ViscosityApp();
