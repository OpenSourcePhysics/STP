 ...
 * are identical
 */
package org.opensourcephysics.stp.ChemicalPotentialDemon;
import org.opensourcephysics.display.*;
import org.opensourcephysics.numerics.*;
import java.awt.*;

public class ChemicalPotentialDemon implements Drawable {
    ...
    for(int i = 0; i<L; i++) {
      for(int j = 0; j<Lp; j++) {
        if(s[i][j]!=0) {
          ...
          int ypix = myWorld.yToPix(j-pmax)-pxRadius;
          g.fillOval(xpix, ypix, 2*pxRadius, 2*pxRadius);
        }
      }
    }
  }
